...
import { columns, parseFields } from "@/config/options/videos.js";
  ...
  data() {
    ...
    return {
      ...
      loadData: (parameter) => {
        ...
        return this.$store.dispatch("list_videos", requestParameters).then((res) => {
  ...
  methods: {
    ...
    delMedia() {
      if (this.checkedKeys.length > 0 && this.view_next) {
        this.asking = true;
        this.sumbit({ ids: this.checkednextKeys, videos: this.selRecord.id }, "del_multi");
        console.log(".");
      } else {
    ...
    oked(vals, form) {
      if (this.view_next) {
        vals.set("videos", this.selRecord.id);
    ...
    sumbit(vals, type, form) {
      ...
      if (this.view_next) {
        ...
        vals["pages"] = { pageNo: table.localPagination.current, pageSize: table.localPagination.pageSize };
        return this.$store.dispatch(type + "_videos", vals).then((res) => {
          // this.onClose();
