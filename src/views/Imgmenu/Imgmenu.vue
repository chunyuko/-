...
import { columns, parseFields } from "@/config/options/imgmenu.js";
import BreadCrumb from "@/views/Components/BreadCrumb";
import { parseFields_img1 } from "@/config/options/img1.js";
let [models, configs] = parseFields();
let [models_img1, configs_img1] = parseFields_img1();
let media_fields = {
  img1: {
    models: models_img1,
    configs: configs_img1,
  },
};
export default {
  provide() {
  ...
  data() {
    ...
    return {
      total: 0,
      dataMedia: { content: [] },
      dataNext: { video: [], videos: [], img1: [], books: [], imagetext: [] },
      destroy: true,
      //第一层
      asking: false,
      eleVisible: false,
      ...
      loadData: (parameter) => {
        ...
        return this.$store.dispatch("list_imgmenu", requestParameters).then((res) => {
  ...
  methods: {
    ...
    delMedia() {
      if (this.checkednextKeys.length > 0 && this.view_next) {
        this.asking = true;
        this.sumbit({ ids: this.checkednextKeys, imgmenu: this.selRecord.id }, "del_multi");
        console.log("..");
      } else {
    ...
    oked(vals, form) {
      if (this.view_next === true) {
        vals.set("imgmenu", this.selRecord.id);
        return this.limitMul(vals, form, ["img"], getImg);
      } else {
        vals.set("imgs", localStorage.getItem("imgs"));
    ...
    sumbit(vals, type, form) {
      ...
      if (this.view_next) {
        ...
        vals["pages"] = { pageNo: table.localPagination.current, pageSize: table.localPagination.pageSize };
        vals["refresh"] = true;
        return this.$store.dispatch(type + "_imgmenu", vals).then((res) => {
          // this.onClose();
          this.tab = type;
